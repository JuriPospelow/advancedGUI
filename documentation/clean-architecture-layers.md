# Clean Architecture Layers — advancedGUI

![Clean Architecture layers](images/clean-architecture-layers.png)
...
| **Adapters** | mqtt-connector, mqtt-scanner, unix-scanner, pino-logger, file-user-store, express-server, ws-bridge, broker, mock-manager | Ports + Core | Ports interfaces |
| **External** | Browser, MQTT devices, Unix sockets, .auth.json, Docker | nothing | Adapters (via network/FS) |

> **Composition root:** `src/main/index.ts` + `src/main/shutdown.ts` wire everything together (process lifecycle, SIGTERM/SIGINT). They are glue, not clean-architecture layer components, so they are not placed in any layer.
